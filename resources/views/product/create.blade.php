...
@section('content')
    <div style="width: 90%; margin: 20px auto; padding: 20px; background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
        <h1 style="font-size: 24px; font-weight: bold; color: #333; text-align: center; margin-bottom: 20px;">Create Product</h1>
        ...
        <form action="{{ route('products.store') }}" method="POST" style="max-width: 600px; margin: 0 auto;">
            @csrf

            <div class="mb-4">
                <label for="name" style="color: #333; font-size: 16px; margin-bottom: 8px; display: block;">Name</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ old('name') }}" required style="border-color: #A688CA;">
            </div>

            <div class="mb-4">
                <label for="description" style="color: #333; font-size: 16px; margin-bottom: 8px; display: block;">Description</label>
                <textarea class="form-control" id="description" name="description" required style="border-color: #A688CA;">{{ old('description') }}</textarea>
            </div>

            <div class="mb-4">
                <label for="price" style="color: #333; font-size: 16px; margin-bottom: 8px; display: block;">Price</label>
                <input type="number" class="form-control" id="price" name="price" step="0.01" value="{{ old('price') }}" required style="border-color: #A688CA;">
            </div>

            <div class="mb-4">
                <label for="stock" style="color: #333; font-size: 16px; margin-bottom: 8px; display: block;">Stock</label>
                <input type="number" class="form-control" id="stock" name="stock" value="{{ old('stock') }}" required style="border-color: #A688CA;">
            </div>
